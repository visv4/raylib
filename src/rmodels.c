...
#define GL_TEXTURE_COORD_ARRAY 0x8078
    ...
    if (mesh.texcoords && material.maps[MATERIAL_MAP_DIFFUSE].texture.id > 0)
    ...
    if (mesh.animNormals)
        rlEnableStatePointer(GL_NORMAL_ARRAY, mesh.animNormals);
...
#define TEXT_BYTES_PER_LINE 20
    ...
    if (mesh.vertices != NULL) // Vertex position (XYZ - 3 components per vertex - float)
    {
        ...
        for (int i = 0; i < mesh.vertexCount * 3 - 1; i++)
            byteCount += sprintf(txtData + byteCount, ((i % TEXT_BYTES_PER_LINE == 0) ? "%.3ff,\n" : "%.3ff, "), mesh.vertices[i]);
    ...
    if (mesh.texcoords != NULL) // Vertex texture coordinates (UV - 2 components per vertex - float)
    {
        ...
        for (int i = 0; i < mesh.vertexCount * 2 - 1; i++)
    ...
    if (mesh.tangents != NULL) // Vertex tangents (XYZW - 4 components per vertex - float)
    {
        ...
        for (int i = 0; i < mesh.vertexCount * 4 - 1; i++)
            byteCount += sprintf(txtData + byteCount, ((i % TEXT_BYTES_PER_LINE == 0) ? "%.3ff,\n" : "%.3ff, "), mesh.tangents[i]);
    ...
    if (mesh.indices != NULL) // Vertex indices (3 index per triangle - unsigned short)
    {
        ...
        for (int i = 0; i < mesh.triangleCount * 3 - 1; i++)
            byteCount += sprintf(txtData + byteCount, ((i % TEXT_BYTES_PER_LINE == 0) ? "%i,\n" : "%i, "), mesh.indices[i]);
...
static void ProcessMaterialsOBJ(Material *materials, tinyobj_material_t *mats, int materialCount)
{
    ...
    for (int m = 0; m < materialCount; m++)
    {
        ...
        if (mats[m].diffuse_texname != NULL)
            materials[m].maps[MATERIAL_MAP_DIFFUSE].texture = LoadTexture(mats[m].diffuse_texname); // char *diffuse_texname; // map_Kd
        else
            materials[m].maps[MATERIAL_MAP_DIFFUSE].color = (Color){(unsigned char)(mats[m].diffuse[0] * 255.0f), (unsigned char)(mats[m].diffuse[1] * 255.0f), (unsigned char)(mats[m].diffuse[2] * 255.0f), 255}; // float diffuse[3];
        materials[m].maps[MATERIAL_MAP_DIFFUSE].value = 0.0f;

        if (mats[m].specular_texname != NULL)
            materials[m].maps[MATERIAL_MAP_SPECULAR].texture = LoadTexture(mats[m].specular_texname);                                                                                                               // char *specular_texname; // map_Ks
        ...
        if (mats[m].bump_texname != NULL)
            materials[m].maps[MATERIAL_MAP_NORMAL].texture = LoadTexture(mats[m].bump_texname); // char *bump_texname; // map_bump, bump
        ...
        if (mats[m].displacement_texname != NULL)
            materials[m].maps[MATERIAL_MAP_HEIGHT].texture = LoadTexture(mats[m].displacement_texname); // char *displacement_texname; // disp
    }
}
#endif
...
Mesh GenMeshCylinder(float radius, float height, int slices)
{
    ...
    if (slices >= 3)
    {
        ...
        for (int i = 0; i < 2 * capTop->npoints; i++)
            capTop->tcoords[i] = 0.0f;
        ...
        for (int i = 0; i < 2 * capBottom->npoints; i++)
            capBottom->tcoords[i] = 0.95f;
...
static Model LoadOBJ(const char *fileName)
{
    ...
    for (unsigned int faceId = 0; faceId < objAttributes.num_faces; faceId++)
    {
        if (faceId >= nextShapeEnd)
        {
            ...
            if (nextShape < objShapeCount)
                ...
                nextShapeEnd = objAttributes.num_face_num_verts; // This is actually the total number of face verts in the file, not faces
            meshIndex++;
        }
    ...
    for (unsigned int faceId = 0; faceId < objAttributes.num_faces; faceId++)
    {
        ...
        if (lastMaterial != -1 && objAttributes.material_ids[faceId] != lastMaterial)
            newMesh = true;
        lastMaterial = objAttributes.material_ids[faceId];
    ...
    for (unsigned int a = 0; a < iqmHeader->num_anims; a++)
    {
        ...
        for (unsigned int j = 0; j < iqmHeader->num_poses; j++)
        {
            ...
            if (iqmHeader->num_joints > 0)
                ...
                memcpy(animations[a].bones[j].name, "ANIMJOINTNAME", 13); // Default bone name otherwise
            animations[a].bones[j].parent = poses[j].parent;
        }

        for (unsigned int j = 0; j < anim[a].num_frames; j++)
            animations[a].framePoses[j] = (Transform *)RL_MALLOC(iqmHeader->num_poses * sizeof(Transform));
...
static BoneInfo *LoadBoneInfoGLTF(cgltf_skin skin, int *boneCount)
{
    *boneCount = (int)skin.joints_count;
    ...
    for (unsigned int i = 0; i < skin.joints_count; i++)
    {
        ...
        if (node.name != NULL)
            strncpy(bones[i].name, node.name, sizeof(bones[i].name) - 1);
...
static void LoadAttachments(Model *model, cgltf_node *node)
{
    if (!model || !node)
    {
        TRACELOG(LOG_ERROR, "No Model or Node given to LoadAttachments()"); 
        return;
    }

    int nodeIsAttachment = 1;

    int fullNameLen = strlen(node->name);
    ...
    int nameLen = fullNameLen - strlen(emptyExt);

    for (int i = nameLen; i < fullNameLen; i++)
    {
        if (node->name[i] != emptyExt[i - nameLen])
        {
            nodeIsAttachment = 0;
            break;
        }
    ...
    if (nodeIsAttachment)
    {
        ...
        {
            // Find the parent bone in the model's bone array
            for (int i = 0; i < model->boneCount; i++)
            {
                if (strcmp(model->bones[i].name, node->parent->name) == 0)
                {
                    e.parentBoneIndex = i;
                    break;
                }
    ...
    for (int i = 0; i < node->children_count; i++){
        LoadAttachments(model, node->children[i]);
    }
}

Model LoadModelFromMemory(const char *fileName, const char* glTFData, const int size){
    // MODIFIED LoadGLTF() to support memory loading 
...
#define LOAD_ATTRIBUTE_CAST(accesor, numComp, srcType, dstPtr, dstType)                                                                                       \
    ...
    // glTF file loading
    int dataSize = size;
    unsigned char *fileData = glTFData;
    ...
    if (result == cgltf_result_success)
    {
        ...
        TRACELOG(LOG_INFO, "    > Buffers count: %i", data->buffers_count);
        TRACELOG(LOG_INFO, "    > Images count: %i", data->images_count);
        TRACELOG(LOG_INFO, "    > Textures count: %i", data->textures_count);

        // supposedly empties are exported as scenes.
        TRACELOG(LOG_INFO, "    > Scenes count: %i", data->scenes_count);
            ...
            for (unsigned int p = 0; p < mesh->primitives_count; p++)
            {
                ...
                for (unsigned int j = 0; j < mesh->primitives[p].attributes_count; j++)
                {
                    ...
                    if (mesh->primitives[p].attributes[j].type == cgltf_attribute_type_position) // POSITION, vec3, float
                    {
                        ...
                        if (model.meshes[meshIndex].vertices != NULL)
                            TRACELOG(LOG_WARNING, "MODEL: [%s] Vertices attribute data already loaded", fileName);
                        else
                        {
                            ...
                            else if ((attribute->type == cgltf_type_vec3) && (attribute->component_type == cgltf_component_type_r_16u))
                            {
                                ...
                                for (unsigned int t = 0; t < attribute->count * 3; t++)
                                    model.meshes[meshIndex].vertices[t] = (float)temp[t];
                    ...
                    else if (mesh->primitives[p].attributes[j].type == cgltf_attribute_type_normal) // NORMAL, vec3, float
                    {
                        ...
                        if (model.meshes[meshIndex].normals != NULL)
                            TRACELOG(LOG_WARNING, "MODEL: [%s] Normals attribute data already loaded", fileName);
                        else
                        {
                            if ((attribute->type == cgltf_type_vec3) && (attribute->component_type == cgltf_component_type_r_32f))
                            {
                            ...
                            else if ((attribute->type == cgltf_type_vec3) && (attribute->component_type == cgltf_component_type_r_16))
                            {
                                ...
                                for (unsigned int t = 0; t < attribute->count * 3; t++)
                                    model.meshes[meshIndex].normals[t] = (float)temp[t];
                            ...
                            else if ((attribute->type == cgltf_type_vec3) && (attribute->component_type == cgltf_component_type_r_8))
                            {
                                ...
                                TRACELOG(LOG_WARNING, "MODEL: [%s] Normals attribute data format not supported, use vec3 float", fileName);
                        }
                    }
                    else if (mesh->primitives[p].attributes[j].type == cgltf_attribute_type_tangent) // TANGENT, vec4, float, w is tangent basis sign
                    {
                        ...
                        if (model.meshes[meshIndex].tangents != NULL)
                            TRACELOG(LOG_WARNING, "MODEL: [%s] Tangents attribute data already loaded", fileName);
                        else
                        {
                            if ((attribute->type == cgltf_type_vec4) && (attribute->component_type == cgltf_component_type_r_32f))
                            {
                                ...
                                TRACELOG(LOG_WARNING, "MODEL: [%s] Tangents attribute data format not supported, use vec4 float", fileName);
                        }
                    }
                    ...
                    else if (mesh->primitives[p].attributes[j].type == cgltf_attribute_type_color) // COLOR_n, vec3/vec4, float/u8n/u16n
                    {
                        ...
                        if (model.meshes[meshIndex].colors != NULL)
                            TRACELOG(LOG_WARNING, "MODEL: [%s] Colors attribute data already loaded", fileName);
                        else
                        {
                            ...
                            else if (attribute->type == cgltf_type_vec4) // RGBA
                            {
                                ...
                                else if (attribute->component_type == cgltf_component_type_r_16u)
                                {
                                    ...
                                    for (unsigned int c = 0; c < attribute->count * 4; c++)
                                        model.meshes[meshIndex].colors[c] = (unsigned char)(((float)temp[c] / 65535.0f) * 255.0f);
                ...
                if ((mesh->primitives[p].indices != NULL) && (mesh->primitives[p].indices->buffer_view != NULL))
                {
                    ...
                    if (model.meshes[meshIndex].indices != NULL)
                        TRACELOG(LOG_WARNING, "MODEL: [%s] Indices attribute data already loaded", fileName);
                    else
                    {
                ...
                {
                    // Copy the name from GLTF Node to Raylib Mesh
                    strncpy(model.meshes[meshIndex].name, data->nodes[i].name, 64);
                    model.meshes[meshIndex].name[63] = '\0'; // Ensure null-termination
                }
                else
                {
                    // Fallback if no name exists
                    sprintf(model.meshes[meshIndex].name, "unnamed_%d", meshIndex);
                }
...
/*********************************************************************************************
    ...
    FEATURES:
      ...
      - Mesh in stances in the glTF file (i.e. same mesh linked from multiple nodes)
...
{
    if (interpolationType >= cgltf_interpolation_type_max_enum)
        return false;
            ...
            *animCount = (int)data->animations_count;
            ...
            for (unsigned int i = 0; i < data->animations_count; i++)
            {
                ...
                if (animData.name != NULL)
                    strncpy(animations[i].name, animData.name, sizeof(animations[i].name) - 1);
            ...
            if (mi != m3d->face[i].materialid)
            {
                ...
                for (j = i, l = vcolor = 0; (j < (int)m3d->numface) && (mi == m3d->face[j].materialid); j++, l++)
                {
                    if (!m3d->vertex[m3d->face[j].vertex[0]].color ||
                        ...
                        !m3d->vertex[m3d->face[j].vertex[2]].color)
                        vcolor = 1;
                }
            ...
            for (j = 0; j < m3d->material[i].numprop; j++)
            {
                ...
                default:
                {
                    if (prop->type >= 128)
                    {
                        ...
                        image.mipmaps = 1;
                        image.format = (m3d->texture[prop->value.textureid].f == 4) ? PIXELFORMAT_UNCOMPRESSED_R8G8B8A8 : ((m3d->texture[prop->value.textureid].f == 3) ? PIXELFORMAT_UNCOMPRESSED_R8G8B8 : ((m3d->texture[prop->value.textureid].f == 2) ? PIXELFORMAT_UNCOMPRESSED_GRAY_ALPHA : PIXELFORMAT_UNCOMPRESSED_GRAYSCALE));
    ...
    *animCount = 0;
    ...
    if (fileData != NULL)
    {
        ...
        if (!m3d || M3D_ERR_ISFATAL(m3d->errcode))
        {
            ...
            TRACELOG(LOG_INFO, "MODEL: [%s] M3D data loaded successfully: %i animations, %i bones, %i skins", fileName,
